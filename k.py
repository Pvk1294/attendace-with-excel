...
class App:
    ...
    def create_excel_sheet(self):
        if not os.path.exists(self.excel_path):
            df = pd.DataFrame(columns=['Serial No', 'Name', 'Present', 'Date/Time'])
    ...
    def accept_register_new_user(self):
        ...
        registered_face_path = os.path.join(self.db_dir, '{}.jpg'.format(name))
        ...
        df = pd.read_excel(self.excel_path)
        serial_no = len(df) + 1
        new_data = {'Serial No': serial_no, 'Name': name, 'Present': 'Yes', 'Date/Time': current_datetime}
    ...
    def mark_attendance(self, name):
        ...
        if name not in df['Name'].tolist():
            serial_no = len(df) + 1
            new_data = {'Name': name, 'Present': 'Yes'}
            new_df = pd.DataFrame([new_data])
            df = pd.concat([df, new_df], ignore_index=True)
            df.to_excel(self.excel_path, index=False)
